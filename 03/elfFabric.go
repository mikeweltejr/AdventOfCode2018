...
func (e elfFabric) getSharedSquareInches() int {
	...
	for _, f := range e {
		...
		calculatePoints(left, top, length, height)
	}

	fmt.Println(len(graphPoints))
	commonPoints := totalSquareInchesInCommon(graphPoints)
	fmt.Println(commonPoints)
	return 0
}

func calculatePoints(left int, top int, length int, height int) [][]int {
	fmt.Println("Left:", left, "Top:", top, "Length:", length, "Height:", height)
	for i := 0; i < length; i++ {
		x := []int{}
		y := []int{}
		x = append(x, i+left)
